...
test -z "$install_list" || yum -y install $install_list >> $logfile 2>&1
...
if [ $OSNAME = "Debian" ]; then
  ...
  yum -y install unrar  >> $logfile 2>&1
fi
...
if ! [ $OSNAME = "Raspbian" ] && [ $(dpkg-query -W -f='${Status}' "ffmpeg" 2>/dev/null | grep -c "ok installed") = 0 ]; then
  ...
  if [ $RELNO = 14 ]; then
    ...
    yum update >> $logfile 2>&1 || error_exit "problem updating package lists"
    yum -y install ffmpeg >> $logfile 2>&1
  elif [ $RELNO = 8 ]; then
    ...
    yum update >> $logfile 2>&1 || error_exit "problem updating package lists"
    yum -y --force-yes install deb-multimedia-keyring >> $logfile 2>&1
    yum -y --force-yes install ffmpeg >> $logfile 2>&1
  else
    yum -y install ffmpeg >> $logfile 2>&1
  fi
fi
...
if [ $(dpkg-query -W -f='${Status}' "vsftpd" 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
  ...
  if [ $RELNO = 12 ]; then
    ...
    yum update >> $logfile 2>&1
    yum -y install vsftpd >> $logfile 2>&1
  elif [ $RELNO = 7 ]; then
